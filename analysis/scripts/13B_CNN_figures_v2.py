...
def load_and_parse_metrics(file_path, column_prefix):
    """Loads a JSON metrics file and extracts precision, recall, and f1-score for each class and average."""
    ...
    for cls in TABLE_ROWS:
        ...
        data[cls] = {
            f"Precision ({column_prefix})": class_metrics.get("precision", 0.0),
            f"Recall ({column_prefix})": class_metrics.get("recall", 0.0),
            f"F1 ({column_prefix})": class_metrics.get("f1-score", 0.0)
        }
...
leaves_df = load_and_parse_metrics(LEAVES_METRICS_PATH, "Leaves")
english_df = load_and_parse_metrics(ENGLISH_METRICS_PATH, "English")
spanish_df = load_and_parse_metrics(SPANISH_METRICS_PATH, "Spanish")

# Corrected: Check for None values to avoid ValueError
if leaves_df is None or english_df is None or spanish_df is None:
    print("Could not generate table due to missing input files.")
else:
    # Concatenate the DataFrames
    combined_df = pd.concat([leaves_df, english_df, spanish_df], axis=1)

    # Round all values to 2 decimal places
    combined_df = combined_df.round(2)
    combined_df.index.name = 'Class/Average'
    
    # Save to CSV
    combined_df.to_csv(TABLE_CSV_PATH)
...
for i, morpheme_class in enumerate(MORPHEME_CLASSES):
    ...
    english_wc_path = ENGLISH_WC_DIR / f"wordcloud_gradcam_idf_{morpheme_class}.png"
    ...
    spanish_wc_path = SPANISH_WC_DIR / f"wordcloud_gradcam_idf_{morpheme_class}.png"
