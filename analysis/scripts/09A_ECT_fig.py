...
for class_idx in range(len(class_names)):
    ...
    if class_idx in real_samples_by_class and len(real_samples_by_class[class_idx]) > 0:
        ...
        # Plot the black contour with a thinner line on top of the ECT image
        if contours:
            # We assume the largest contour is the main leaf outline
            main_contour = max(contours, key=len)
            ax.plot(main_contour[:, 1], main_contour[:, 0], color='black', linewidth=0.5)
