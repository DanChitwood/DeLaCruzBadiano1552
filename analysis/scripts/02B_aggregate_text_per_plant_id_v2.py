...
def clean_and_normalize_text(text: str, language: str) -> str:
    ...
    3. Removes stopwords.
    """
    # 1. First, perform all synonym/term replacements in one pass.
    #    This is done by building a single regex pattern and using a custom
    #    replacement function, which prevents the multiple-replacement bug.

    # Function to get the official name for a matched term
    def replacer(match):
        term = match.group(0).lower() # Get the matched term, lowercase it
        # Return the official name from our global map, or the original term if not found
        return GLOBAL_SYNONYM_TO_OFFICIAL_NAME_MAP.get(term, term)

    # Sort the search terms by length, in descending order.
    # This is critical to ensure that longer, more specific names (e.g., 'xaltomatl')
    # are matched before shorter, more general ones (e.g., 'tomatl').
    ...
    # `re.escape` handles any special characters in the names (e.g., hyphens)
    # The `(?i)` flag makes the whole pattern case-insensitive.
    pattern = '|'.join(re.escape(term) for term in sorted_terms)
    
    # Perform the replacement using re.sub with our custom replacer function.
    # The replacer function will be called for every match found.
    processed_text = re.sub(f'(?i)({pattern})', replacer, text)
    ...
    # 4. Normalize whitespace and join
    ...
    for (plant_id, official_name), group_df in grouped:
        ...
        for _, row in group_df.iterrows():
            ...
            if raw_subchapter_content:
                # The text is now cleaned and normalized here, with the new, robust function
